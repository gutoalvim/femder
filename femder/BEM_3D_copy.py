...
import femder as fd
def ext_bool_coord_from_geo(path_to_geo,geo_axis,coord_axis,mSize,dilate_amount,min_a,max_a,min_b,max_b):
    try:  
        import gmsh
    except :
    ...
    if geo_axis == 'z':
        gmsh.model.occ.addPoint(min_a-1,min_b-1 , coord_axis, 0., 3001)
        gmsh.model.occ.addPoint(max_a+1,min_b-1 , coord_axis, 0., 3002)
        gmsh.model.occ.addPoint(max_a+1,max_b+1 , coord_axis, 0., 3003)
        gmsh.model.occ.addPoint(min_a-1,max_b+1 , coord_axis, 0., 3004)
        ...
        gmsh.model.occ.cut([(2,15000)],tgv,-1,True,True)

        # gmsh.model.occ.dilate([(2,15000)],(min_a+max_a)/2, (min_b+max_b)/2, 0, dilate_amount,dilate_amount, 0 )
    if geo_axis == 'y':
        gmsh.model.occ.addPoint(min_a-1,coord_axis , min_b-1, 0., 3001)
        gmsh.model.occ.addPoint(max_a+1,coord_axis , min_b-1, 0., 3002)
        gmsh.model.occ.addPoint(max_a+1,coord_axis , max_b+1, 0., 3003)
        gmsh.model.occ.addPoint(min_a-1,coord_axis , max_b+1, 0., 3004)
        gmsh.model.occ.addLine(3001, 3004, 3001)
        gmsh.model.occ.addLine(3004, 3003, 3002)
        gmsh.model.occ.addLine(3003, 3002, 3003)
        gmsh.model.occ.addLine(3002, 3001, 3004)
        ...
        # gmsh.model.occ.dilate([(2,15000)],(min_a+max_a)/2, 0, (min_b+max_b)/2,dilate_amount, 0, dilate_amount )
    if geo_axis == 'x':
        gmsh.model.occ.addPoint(coord_axis,min_a-1, min_b-1, 0., 3001)
        gmsh.model.occ.addPoint(coord_axis,max_a+1 , min_b-1, 0., 3002)
        gmsh.model.occ.addPoint(coord_axis,max_a+1 , max_b+1, 0., 3003)
        gmsh.model.occ.addPoint(coord_axis,min_a-1 , max_b+1, 0., 3004)
        gmsh.model.occ.addLine(3001, 3004, 3001)
        gmsh.model.occ.addLine(3004, 3003, 3002)
        gmsh.model.occ.addLine(3003, 3002, 3003)
        gmsh.model.occ.addLine(3002, 3001, 3004)
        ...
        # gmsh.model.occ.dilate([(2,15000)],0, (min_b+max_b)/2, (min_b+max_b)/2, 0, dilate_amount, dilate_amount)
    ...
    elem = np.array(nodeTagss,dtype=int).reshape(-1,3)-1
    gmsh.finalize()
    return nos,elem
...
class BEM3D:
    ...
    def pressure_field(self,dxy=0,dyz=0,dxz=0,fieldtype='pTotal', Pmin=None, frequencies=[60], Pmax=None, axis=['xy', 'yz', 'xz', 'boundary'],
                       axis_visibility={'xy': True, 'yz': True, 'xz': 'legendonly', 'boundary': True},
                       coord_axis={'xy': None, 'yz': None, 'xz': None, 'boundary': None}, dilate_amount=1.1,
                       xminmax = [-1,1],yminmax = [-1,1],zminmax = [-1,1], gridsize=0.1, gridColor="rgb(230, 230, 255)",
        ...
        if coord_axis['boundary'] is None:
            coord_axis['boundary'] = (zminmax[0] + zminmax[1]) / 2
        
        if 'xy' in axis:
            xy_nos,xy_elem= ext_bool_coord_from_geo(self.path_to_geo,'z',coord_axis['xy'],gridsize,1.01,xminmax[0],xminmax[1],yminmax[0],yminmax[1])
            Rxy = fd.Receiver()
            Rxy.coord = xy_nos
        if 'yz' in axis:
            yz_nos,yz_elem = ext_bool_coord_from_geo(self.path_to_geo,'x',coord_axis['yz'],gridsize,1.01,yminmax[0],yminmax[1],zminmax[0],zminmax[1])
            Ryz = fd.Receiver()
            Ryz.coord = yz_nos
    
        if 'xz' in axis:
            xz_nos,xz_elem = ext_bool_coord_from_geo(self.path_to_geo,'y',coord_axis['yz'],gridsize,1.01,xminmax[0],xminmax[1],zminmax[0],zminmax[1])
            Rxz = fd.Receiver()
            Rxz.coord = xz_nos
        self.xy = xy_nos
        ...
        prog = 0
        # for fi in f_rangeuencies:
        # if len(f_rangeuencies) > 1:
        #     progress_bar(prog / len(f_rangeuencies))
            
        fi = np.argwhere(self.freq==frequencies)[0][0]
        # boundData = self.bem.simulation._solution_data[idx]

        k = np.pi*2*self.freq[fi]/self.c0
        ...
        if 'xy' in axis:
            ...
            pT_xy,pScat_xy =  self.evaluate(Rxy)
            pT_xy = pT_xy[fi]
            pScat_xy = pScat_xy[fi]
            
            if fieldtype == 'pTotal':   
                values_xy = np.real((pT_xy)).flatten()
            elif fieldtype == 'pScat':
                values_xy = np.real(np.abs(pScat_xy)).flatten()
            elif fieldtype == 'pInc':
                values_xy = np.real(p2SPL(np.real(pT_xy-pScat_xy))).flatten()
                                    
        if 'yz' in axis:             
            grid_pts_yz = yz_nos
            grid_pts_yz[:,2] = grid_pts_yz[:,2]+dyz
            R = fd.Receiver()
            R.coord = grid_pts_yz
            pT_yz,pScat_yz =  self.evaluate(Ryz)
            pT_yz = pT_yz[fi]
            pScat_yz = pScat_yz[fi]
            if fieldtype == 'pTotal':   
                values_yz = np.real((pT_yz)).flatten()
            elif fieldtype == 'pScat':
                values_yz = np.real(np.abs(pScat_yz)).flatten()
            elif fieldtype == 'pInc':
                values_yz = np.real(p2SPL(np.real(pT_yz-pScat_yz))).flatten()
        if 'xz' in axis:
            grid_pts_xz = xz_nos
            grid_pts_xz[:,2] = grid_pts_xz[:,2]+dxz
            R = fd.Receiver()
            R.coord = grid_pts_xz
            pT_xz,pScat_xz =  self.evaluate(Rxz)
            pT_xz = pT_xz[fi]
            pScat_xz = pScat_xz[fi]

            if fieldtype == 'pTotal':   
                values_xz = np.real((pT_xz)).flatten()
            elif fieldtype == 'pScat':
                values_xz = np.real(np.abs(pScat_xz)).flatten()
            elif fieldtype == 'pInc':
                values_xz = np.real(p2SPL(np.real(pT_xz-pScat_xz))).flatten()
        if 'boundary' in axis:     

            values_boundary = self.pC[fi]
        # Plotting
        ...
        if Pmin is None:
            Pmin = np.amin(values_xy)
        if Pmax is None:
            Pmax = np.amax(values_xy)
        ...
        if 'boundary' in axis:
            ...
            fig.add_trace(go.Mesh3d(x=vertices[0, :], y=vertices[1, :], z=vertices[2, :],
                                    ...
                                    colorscale=colormap, intensitymode='cell', name='Boundary', showlegend=showlegend,
        ...
        if saveFig:
            if filename is None:
                for camera in camera_angles:
                    ...
                    fig.write_image(f'_3D_pressure_plot_{camera}_{int(self.f_range[fi])}Hz.png', scale=2)
            else:
